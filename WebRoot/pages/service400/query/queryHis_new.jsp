   		...
   		$(function() {
   			$("#queryBtn").click(function(){
				$('#tt').datagrid('load',{  // 向服务器传递的参数
					...
					closestarttime:$('#closestarttime').val(),
					closeendtime:$("#closeendtime").val()
			        }); 
			})
			...
			$('#tt').datagrid({
				...
				url:'<%=basePath%>query400Action_queryPageJson.action',
				...
				columns: [[
					  ...
					  {field:'operation',title:'操作',width:fixColumnWidth(0.1500),
						   formatter: function(val,node){
						    	        return "<a class=\"operation-a\" id=\"operation\" href=${path}/query400Action_download.action?tsf.id="+node.id+">申请信息</a>";
						   }
			   		  }
				]],
				//下面 定义 分页配置 ：
	...
	<div class="search">
    	<table onKeyUp="bindQuery();">
    	  <tr>
    	   ...
    	   <td><input class="t-text" id="sendperson" type="text" value=""></td>
    	   <td>申请类型:</td>
             <td>
             	<select id="type">
	             <option value="">全部</option>
	             <option value="A">400号码申请</option>
		  ...
		  <tr>
            ...
            <td class="t-title">完成时间:</td>
            <td>
            	<input class="t-text" id="closestarttime" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="closestarttime" type="text" value="">
            	—
            	<input class="t-text" id="closeendtime" readonly="readonly" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="closeendtime" type="text" value="">
            </td>
            </tr>
            <tr>
            <td class="t-title">申请号码:</td>
    	   <td><input class="t-text" id="phonenum" type="text" value=""></td>
            <td>
		...
		<div class="search-list">
        	<span class="list-title">400号码,IVR及工作组维护申请历史记录列表</span>
