...
<head>
	...
	function openaddwindow(){
		...
		var iTop = (window.screen.availHeight-30-iHeight)/2;       //获得窗口的垂直位置;
  	    ...
  	    //window.open ("<%=basePath%>pages/service400/num400/add400num.jsp", "", "height=100, width=100, top="+iTop+",left="+iLeft+",toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no")
        var num=window.showModalDialog("<%=basePath%>pages/service400/num400/add400num.jsp", "", "dialogHeight=250px, dialogWidth=50px, top=0,left=0,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no") 
        if(num)
        	window.location.href="<%=basePath%>num400Action_to400page.action";
  		}
	...
	function checkAddNum(){
		...
		var price = $("#price").val();
		//re = /^\d{2,4}(\.\d{1,2})?$/;
		var re=/^(0|[1-9][0-9]{0,5})(.[0-9]{1,2})?$/;
		if(parseFloat(price)==0){
			  $.messager.alert("提示", "月最低消费额输入不正确，不能为0！");
			  flag = false;
			  return flag;
		}else if(!re.test(price)){
			$.messager.alert("提示", "月最低消费额输入不正确，消费额整数长度只能为1到6位，且小数点后最多2位！");
			flag = false;
			return flag;
		}
		if(flag){
	...
	<form id="addNum" action="<%=basePath%>num400Action_add400.action">
			<table>
				<tr>
					...
					<a href="javascript:void(0)" class="but-cancel" onclick="closeAdd()" plain="true">取消</a>
					</div>
					</td>
				</tr>
			</table>
	...
	<div class="search">
		...
		<%--<a href='javascript:void(0)' onclick="openaddwindow()" class='easyui-linkbutton' iconCls='icon-add' plain='true'>添加</a>
		--%></div>
	</body>
</html>
